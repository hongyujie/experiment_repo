# 四层CNN模型 - 基于cnn_improved.py进行修改
# 修改内容：
# 1. 模型结构优化：
#    - 将原有的三层卷积网络扩展为四层卷积网络
#    - 调整网络宽度：32→64→128→256
#    - 调整全连接层的输入特征数量以适应四层卷积的输出
# 2. 正则化：
#    - 保留原有Dropout层防止过拟合
# 3. 输出：
#    - 移除acc和loss图片生成功能
...
class network(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(# 用该方法更方便
            ...
            nn.Flatten(),
            # 全连接层：Dropout层防止过拟合
            nn.Linear(in_features=256, out_features=256),  # 输入特征由1152改为256 (1x1x256)
            nn.ReLU(),
            nn.Dropout(p=0.5),  
...
for i in range(epoch):# 训练和测试一起进行
    ...
    console.print(f"[bold magenta]第{i+1}轮测试数据集loss:{test_loss}，准确率:{test_accuracy:.4f}[/bold magenta]")
    total_test_step += 1

# 计算测试准确率的平均值
avg_test_accuracy = sum(test_accuracy_list) / len(test_accuracy_list)
console.print(f"[bold green]所有测试轮次的平均准确率: {avg_test_accuracy:.4f}[/bold green]")
console.print(f"[bold blue]最高测试准确率: {max(test_accuracy_list):.4f} (第{test_accuracy_list.index(max(test_accuracy_list))+1}轮)[/bold blue]")