...
def compute_fft_features(window, sample_rate=1240):
    ...
    # 频带能量特征
    low_band = np.sum(fft_magnitude[(freqs >= 0) & (freqs < 10)])  # 0-10Hz
    mid_band = np.sum(fft_magnitude[(freqs >= 10) & (freqs < 100)])  # 10-100Hz
    high_band = np.sum(fft_magnitude[freqs >= 100])  # >100Hz

    # 归一化
    total = low_band + mid_band + high_band + 1e-8
    return np.array([low_band/total, mid_band/total, high_band/total], dtype=np.float32)

def split_and_window_with_fft(df, window_size=200, step_size=10, train_ratio=0.7):
    """
    先按时间切分原始数据，再分别滑窗，同时提取FFT特征
    """
    X_train, X_test, y_train, y_test = [], [], [], []
    F_train, F_test = [], []  # FFT特征
...
class SignalDatasetWithFFT(Dataset):
    """PyTorch数据集封装（包含FFT特征）"""
...
class CNNLSTMWithFreqClassifier(nn.Module):
    """
    CNN-LSTM + 频域特征 混合网络
    改进点：添加FFT频域特征分支，帮助区分UDP和ICMP
    """
    def __init__(self, num_classes=4, dropout=0.5):
        super(CNNLSTMWithFreqClassifier, self).__init__()
    ...
    def forward(self, x, freq_features):
        ...
        # 频域特征提取
        freq_out = self.freq_fc(freq_features)  # [batch, 16]

        # 特征融合
        combined = torch.cat([time_features, freq_out], dim=1)  # [batch, 80]
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+FFT 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + FFT频域特征 四分类任务
    改进点：添加频域特征分支，帮助区分UDP和ICMP
    """
    print('=' * 60)
    print('CNN-LSTM+FFT频域特征 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    print('\n2. 先切分后滑窗（包含FFT特征提取）...')
    X_train, X_test, F_train, F_test, y_train, y_test = split_and_window_with_fft(
        df, window_size=window_size, step_size=step_size, train_ratio=0.7
    )
    print(f'训练集: {X_train.shape}, 测试集: {X_test.shape}')
    print(f'FFT特征 - 训练集: {F_train.shape}, 测试集: {F_test.shape}')
    ...
    print('\n3. 创建数据加载器...')
    train_dataset = SignalDatasetWithFFT(X_train, F_train, y_train, augment=True)
    test_dataset = SignalDatasetWithFFT(X_test, F_test, y_test, augment=False)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    print('\n4. 构建CNN-LSTM+FFT模型...')
    model = CNNLSTMWithFreqClassifier(num_classes=4, dropout=0.5).to(device)
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_freq_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_freq_1240Hz.png')
