...
class CNNLSTMWithTCPBranch(nn.Module):
    """
    CNN-LSTM + TCP专用卷积分支
    为TCP类别设计专门的特征提取分支，使用更大的卷积核捕获更宽特征
    """
    def __init__(self, num_classes=4, dropout=0.5):
        super(CNNLSTMWithTCPBranch, self).__init__()
        ...
        # TCP专用卷积分支（更大的卷积核，捕获更宽特征）
        self.tcp_branch = nn.Sequential(
            nn.Conv1d(in_channels=64, out_channels=32, kernel_size=7, padding=3),  # 大卷积核
            nn.BatchNorm1d(32),
            nn.ReLU(),
    ...
    def forward(self, x, labels=None):
        ...
        # TCP专用分支处理
        tcp_feat = self.tcp_branch(x)  # [batch, 16, 1]
        tcp_feat = tcp_feat.squeeze(-1)  # [batch, 16]

        # 为每个时间步复制TCP特征
        seq_len = x.size(2)
        tcp_feat_expanded = tcp_feat.unsqueeze(1).expand(-1, seq_len, -1)  # [batch, seq_len, 16]

        # 转换通用特征维度
        x = x.permute(0, 2, 1)  # [batch, seq_len, 64]

        # 融合通用特征和TCP专用特征
        combined = torch.cat([x, tcp_feat_expanded], dim=-1)  # [batch, seq_len, 80]

        # 特征融合
        fused = self.fusion(combined)  # [batch, seq_len, 64]

        # LSTM处理
        lstm_out, _ = self.lstm(fused)  # [batch, seq_len, 64]

        # 取最后一个时间步
        lstm_out = lstm_out[:, -1, :]  # [batch, 64]
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+TCP-Branch 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + TCP专用卷积分支
    改进点：为TCP类别设计专门的特征提取分支，使用更大的卷积核捕获更宽特征
    """
    print('=' * 60)
    print('CNN-LSTM+TCP专用分支 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    print('\n4. 构建CNN-LSTM+TCP分支模型...')
    model = CNNLSTMWithTCPBranch(num_classes=4, dropout=0.5).to(device)
    ...
    plot_training_curves(train_losses, train_accs, test_accs,
                         'img/training_curves_tcp_branch_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred,
                          'img/confusion_matrix_tcp_branch_1240Hz.png')
