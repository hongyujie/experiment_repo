...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        """
        inputs: [batch, num_classes] - 模型输出（未经过softmax）
        targets: [batch] - 真实标签
        """
        # 计算交叉熵
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+FocalLoss 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + Focal Loss 四分类任务
    改进点：使用Focal Loss替代CrossEntropy，更好地处理类别不平衡
    """
    print('=' * 60)
    print('CNN-LSTM+FocalLoss 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    print('\n2. 先切分后滑窗...')
    X_train, X_test, y_train, y_test = split_and_window(df, window_size=window_size, step_size=step_size,
                                                        train_ratio=0.7)
    ...
    alpha = torch.FloatTensor([1.0, 1.0, 2.0, 1.0]).to(device)
    gamma = 2.0
    print(f'\nFocal Loss参数: alpha={alpha.cpu().numpy()}, gamma={gamma}')
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_focal_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_focal_1240Hz.png')
