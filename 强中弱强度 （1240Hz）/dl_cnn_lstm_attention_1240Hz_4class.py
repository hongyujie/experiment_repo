...
class SelfAttention(nn.Module):
    ...
    def forward(self, lstm_output):
        ...
        return: [batch, hidden_dim] - 加权后的特征
        """
        # 计算attention权重
        attn_weights = self.attention(lstm_output)  # [batch, seq_len, 1]
        attn_weights = F.softmax(attn_weights, dim=1)  # 在seq_len维度上归一化
...
class CNNLSTMAttentionClassifier(nn.Module):
    """
    CNN-LSTM + Self-Attention 混合网络
    改进点：添加注意力机制，让模型关注最有区分性的时间特征
    """
    def __init__(self, num_classes=4, dropout=0.5):
        super(CNNLSTMAttentionClassifier, self).__init__()
        ...
        # Self-Attention层
        self.attention = SelfAttention(hidden_dim=64)
    ...
    def forward(self, x):
        ...
        # LSTM处理
        lstm_out, _ = self.lstm(x)  # [batch, 50, 64]
        
        # Attention加权
        context, attn_weights = self.attention(lstm_out)  # [batch, 64]
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+Attention 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + Self-Attention 四分类任务
    改进点：添加注意力机制，同时保留类别权重平衡
    """
    print('=' * 60)
    print('CNN-LSTM+Self-Attention 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    model = CNNLSTMAttentionClassifier(num_classes=4, dropout=0.5).to(device)
    ...
    print(f'\n类别权重: 正常={class_weights[0]:.1f}, TCP={class_weights[1]:.1f}, UDP={class_weights[2]:.1f}, ICMP={class_weights[3]:.1f}')
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_attention_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_attention_1240Hz.png')
