...
def compute_spectrum_features(window, sample_rate=1240, n_bands=16):
    ...
    # 计算每个频段的能量
    max_freq = sample_rate / 2  # 奈奎斯特频率 = 620Hz
    ...
    features = np.concatenate([
        features,  # 16维频段能量
        [np.mean(features), np.std(features), np.max(features)]  # 3维统计特征
    ])

    return features

def split_and_window_with_spectrum(df, window_size=200, step_size=10, train_ratio=0.7):
    """
    先按时间切分原始数据，再分别滑窗，同时提取精细频谱特征
    """
...
class SignalDatasetWithSpectrum(Dataset):
    """PyTorch数据集封装（包含精细频谱特征）"""
...
class CNNLSTMWithSpectrumClassifier(nn.Module):
    """
    CNN-LSTM + 精细频谱特征 混合网络
    改进点：使用16个频段的精细频谱特征 + 统计特征，共19维
    """
    def __init__(self, num_classes=4, dropout=0.5):
        super(CNNLSTMWithSpectrumClassifier, self).__init__()
    ...
    def forward(self, x, spectrum_features):
        # 时域特征提取
        ...
        # 频谱特征提取
        spectrum_out = self.spectrum_fc(spectrum_features)  # [batch, 32]

        # 特征融合
        combined = torch.cat([time_features, spectrum_out], dim=1)  # [batch, 96]
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+Spectrum 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + 精细频谱特征 四分类任务
    改进点：使用16个频段的精细频谱特征
    """
    print('=' * 60)
    print('CNN-LSTM+精细频谱特征 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    print('\n2. 先切分后滑窗（包含精细频谱特征提取）...')
    X_train, X_test, S_train, S_test, y_train, y_test = split_and_window_with_spectrum(
        df, window_size=window_size, step_size=step_size, train_ratio=0.7
    )
    print(f'训练集: {X_train.shape}, 测试集: {X_test.shape}')
    print(f'频谱特征 - 训练集: {S_train.shape}, 测试集: {S_test.shape}')
    ...
    print('\n3. 创建数据加载器...')
    train_dataset = SignalDatasetWithSpectrum(X_train, S_train, y_train, augment=True)
    test_dataset = SignalDatasetWithSpectrum(X_test, S_test, y_test, augment=False)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    print('\n4. 构建CNN-LSTM+Spectrum模型...')
    model = CNNLSTMWithSpectrumClassifier(num_classes=4, dropout=0.5).to(device)
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_spectrum_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_spectrum_1240Hz.png')
