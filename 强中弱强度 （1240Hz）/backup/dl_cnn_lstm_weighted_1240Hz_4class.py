...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+Weighted 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + 类别权重平衡
    改进点：对UDP类别(标签2)增加权重，强制模型学习区分UDP和ICMP
    """
    print('=' * 60)
    print('CNN-LSTM+类别权重平衡 1240Hz电流信号四分类任务')
    print('=' * 60)

    window_size = 400  # 从300增加到400，捕获更长时序特征
    step_size = 10     # 保持步长不变
    batch_size = 128
    num_epochs = 50
    ...
    # 类别权重：最佳配置
    # 增加UDP(类别2)的权重，解决UDP被误判为ICMP的问题
    class_weights = torch.FloatTensor([1.0, 1.0, 2.0, 1.0]).to(device)  # 正常:1, TCP:1, UDP:2, ICMP:1
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_weighted_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_weighted_1240Hz.png')
