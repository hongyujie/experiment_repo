...
class SignalDataset(Dataset):
    """
    PyTorch数据集封装 - 增强版数据增强
    针对TCP误判问题，增加更多数据增强策略
    """
    ...
    def time_warp(self, x, warp_range=0.1):
        """时间扭曲 - 对时间轴进行轻微扭曲"""
        seq_len = x.shape[-1]
        # 生成扭曲因子
        warp_factor = torch.FloatTensor(1).uniform_(
        ...
        warped_indices = torch.clamp(warpped_indices, 0, seq_len - 1)
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-LSTM+Augmented 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-LSTM + 增强数据增强
    改进点：使用多种数据增强策略，提升模型泛化能力，减少TCP误判
    """
    print('=' * 60)
    print('CNN-LSTM+增强数据增强 1240Hz电流信号四分类任务')
    print('=' * 60)
    ...
    plot_training_curves(train_losses, train_accs, test_accs,
                         'img/training_curves_augmented_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred,
                          'img/confusion_matrix_augmented_1240Hz.png')
