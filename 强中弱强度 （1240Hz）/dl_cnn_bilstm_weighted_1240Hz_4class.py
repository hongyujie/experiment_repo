...
class CNNBiLSTMClassifier(nn.Module):
    """
    CNN-BiLSTM混合网络（双向LSTM版本）
    改进：使用双向LSTM捕获前后向时序依赖，提升TCP识别率
    """
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (CNN-BiLSTM+Weighted 1240Hz)', fontsize=16, pad=20)
...
def main():
    """
    主函数：CNN-BiLSTM + 类别权重平衡
    改进点：使用双向LSTM捕获前后向时序依赖，提升TCP识别率
    """
    print('=' * 60)
    print('CNN-BiLSTM+类别权重平衡 1240Hz电流信号四分类任务')
    print('改进：双向LSTM捕获前后向时序依赖')
    print('=' * 60)

    window_size = 300  # 保持300，捕获更长时序特征
    step_size = 10
    batch_size = 128
    num_epochs = 50
    ...
    print('\n4. 构建CNN-BiLSTM模型...')
    model = CNNBiLSTMClassifier(num_classes=4, dropout=0.5).to(device)
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_bilstm_1240Hz.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_bilstm_1240Hz.png')
