...
## 13. 当发生提交冲突的时
情况是这样的：当a电脑修改了文件，git push提交到github仓库，b电脑接着也修改了这个文件，而且还是修改的同一行，同样git push提交到github仓库，那么就会发生提交冲突。b电脑会显示提交失败，需要手动解决冲突。这个时候的流程是：
```bash
1. 先执行git pull命令，拉取远端的代码到本地仓库。
2. 解决冲突。
3，再执行git add .和git commit -m "解决冲突"命令
4. 之后执行git push命令，将本地仓库的代码推送到远端仓库
```
第一步中，使用git pull拉取代码的时候，会是这样的情况：
![alt text](image-23.png)
意思是：你执行 git pull 后，Git 尝试自动合并 readme.md（这个就是有冲突的文件），但本地和远程对同一部分内容做了修改，导致自动合并失败，出现了内容冲突（CONFLICT (content)）。接着打开readme.md文件，手动解决冲突。在vscode中会是这样的：
![alt text](image-24.png)
```
可以采用下面的方法来解决：
方式 1：VSCode 快捷操作
直接点击编辑器上方的按钮，如果是选择「采用当前更改」按钮：
它会自动删除 ======= 到 >>>>>>> 的远程代码，以及所有冲突标记（<<<<<<< HEAD、=======、>>>>>>>）
只保留你本地的 11111111111111111111111111111111
方式 2：手动编辑文件
打开 readme.md，按下面规则修改：
保留：11111111111111111111111111111111
删除：
<<<<<<< HEAD (当前更改)
=======
从 测试功能测试功能... 到 >>>>>>> 699892085482a160323ca286ccb8666f88bc1b90 (传入的更改) 这整段内容
保存文件
```
之后再执行后面的步骤就可以解决冲突。