...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (二分类: 正常vs攻击)', fontsize=14, pad=20)
...
def main():
    """
    主函数：CNN-LSTM二分类（正常 vs 攻击）
    """
    print('=' * 60)
    print('CNN-LSTM二分类任务: 正常 vs 攻击')
    print('=' * 60)
    ...
    print(f'归一化 - 均值: {data_mean:.2f}, 标准差: {data_std:.2f}')
    ...
    print('\n4. 构建CNN-LSTM模型...')
    model = CNNLSTMClassifier(num_classes=2, dropout=0.5).to(device)
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves_binary.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_binary.png')
