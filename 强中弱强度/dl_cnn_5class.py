...
class CNN1D(nn.Module):
    """
    一维卷积神经网络用于时序信号分类（轻量版，防过拟合）

    架构：
    - 2层卷积块（Conv1D + BatchNorm + ReLU + MaxPool）
    - 自适应平均池化（AdaptiveAvgPool）
    - 全连接层分类器（Dropout=0.7）
    """
...
def plot_confusion_matrix(y_true, y_pred, save_path):
    ...
    ax.set_title('混淆矩阵 (1D-CNN)', fontsize=16, pad=20)
...
def main():
    ...
    print('\n7. 保存图表...')
    plot_training_curves(train_losses, train_accs, test_accs, 'img/training_curves.png')
    plot_confusion_matrix(y_true, y_pred, 'img/confusion_matrix_CNN.png')
