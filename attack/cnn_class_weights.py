...
# 超参数
model_name = "CNN_Class_Weights"
batch_size = 128
epoch = 25
learning_rate = 0.001
dropout = 0
length = 64
use_class_weights = True  # 是否使用类别权重FalseTrue
...
# ----------训练与验证----------
def train(model, optimizer, train_loader, class_weights=None):
    model.train()
    acc = AccMectric()
    ...
    for data, labels in train_loader:
        ...
        loss = criterion(o, y)# 计算损失函数（考虑类别权重）
...
def validate(model, test_loader, class_weights=None):
    model.eval()
    acc = AccMectric()
    pred, targets = [], []
...
if use_class_weights:
    ...
    # 计算权重比例（基于样本数量的倒数）
    weight_class0 = n_class1 / (n_class0 + n_class1)  # 少数类的权重
    weight_class1 = n_class0 / (n_class0 + n_class1)  # 多数类的权重
    
    # 归一化权重，使权重和为类别数
    class_weights = torch.tensor([weight_class1, weight_class0]).double().to(device)
    
    print(f"训练集标签分布: 标签0={n_class0}个, 标签1={n_class1}个")
    print(f"类别权重设置: 标签0={class_weights[0]:.3f}, 标签1={class_weights[1]:.3f}")
else:
    class_weights = None
    print("未使用类别权重")
...
for i in range(epoch):
    train_acc_current, train_loss = train(model, optimizer, train_loader, class_weights)
    test_acc_current, pred, targets, test_loss = validate(model, test_loader, class_weights)
...
heatmap(label_true=targets,
        label_pred=pred,
        label_name=[0, 1],
        title="Confusion Table of CNN_Class_Weights",
