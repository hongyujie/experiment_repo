...
def train_nb_model(X_train, X_test, y_train, y_test):
    ...
    # 优化分类阈值 - 找到最大化tpr-fpr的阈值
    print("\n优化分类阈值...")
    fpr, tpr, thresholds = roc_curve(y_test, y_pred_proba)
    # 计算tpr-fpr的差值，找到最大值对应的阈值
...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    plt.title('时序朴素贝叶斯模型ROC曲线')
    plt.legend(loc="lower right")
    plt.grid(True, alpha=0.3)
    
    # 保存图片到fig文件夹
    plt.savefig('fig/roc_nb_temporal.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.title('Confusion Table of Temporal Naive Bayes')
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/confusion_matrix_nb_temporal.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def main():
    ...
    # 7. 训练朴素贝叶斯模型
    print("\n步骤7: 训练朴素贝叶斯模型")
    y_pred, y_pred_proba = train_nb_model(
    ...
    print("\n=== 模型训练完成 ===")
    print("ROC曲线已保存为: fig/roc_nb_temporal.png")
    print("混淆矩阵已保存为: fig/confusion_matrix_nb_temporal.png")
