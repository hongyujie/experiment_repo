...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/roc_knn_combined.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/confusion_matrix_knn_combined.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def main():
    """
    主函数：执行完整的knn模型训练流程（混合采样）
    """
    print("=== knn二分类模型训练（混合采样） ===\n")
    ...
    # 5. 训练knn模型
    print("\n步骤5: 训练knn模型")
    y_pred, y_pred_proba = train_knn_model(X_train, X_test, y_train, y_test)
    ...
    print("\n=== 模型训练完成 ===")
    print("ROC曲线已保存为: fig/roc_knn_combined.png")
    print("混淆矩阵已保存为: fig/confusion_matrix_knn_combined.png")
