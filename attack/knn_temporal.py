...
def train_knn_model(X_train, X_test, y_train, y_test):
    ...
    knn_model = KNeighborsClassifier(
        n_neighbors=7,               # 调整K值
        weights='distance',          # 使用距离权重，更关注近邻
        metric='minkowski',          # 使用闵可夫斯基距离，结合L1和L2
        p=2,                         # p=2等价于欧几里得距离
        algorithm='auto',            # 自动选择最佳算法
        leaf_size=20,                # 调整叶节点大小
        n_jobs=-1                    # 使用所有CPU核心
    )
...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    plt.title('时序KNN模型ROC曲线')
    plt.legend(loc="lower right")
    plt.grid(True, alpha=0.3)
    
    # 保存图片到fig文件夹
    plt.savefig('fig/roc_knn_temporal.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.title('Confusion Table of Temporal KNN')
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/confusion_matrix_knn_temporal.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def main():
    ...
    # 7. 训练knn模型
    print("\n步骤7: 训练knn模型")
    y_pred, y_pred_proba = train_knn_model(
    ...
    print("\n=== 模型训练完成 ===")
    print("ROC曲线已保存为: fig/roc_knn_temporal.png")
    print("混淆矩阵已保存为: fig/confusion_matrix_knn_temporal.png")
