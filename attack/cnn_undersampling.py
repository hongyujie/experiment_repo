...
"""
CNN模型 - 下采样版本（随机下采样）
通过下采样多数类来平衡数据分布，但保持足够的数据量
下采样前 - 标签0: 11430, 标签1: 2897
下采样后 - 标签0: 5715, 标签1: 2897
"""
...
sampling_strategy = 0.5  # 多数类保留50%的样本（确保数据量足够）
...
def apply_undersampling(X_train, y_train):
    ...
    print(f"下采样前 - 标签0: {n_class0}, 标签1: {n_class1}")
    
    # 计算下采样后的目标数量（确保数据量足够）
    target_n_class0 = int(n_class0 * sampling_strategy)  # 保留50%的多数类样本
    target_n_class1 = n_class1  # 保留所有少数类样本
    
    # 设置下采样策略
    sampling_dict = {0: target_n_class0, 1: target_n_class1}
    
    # 应用随机下采样
    undersampler = RandomUnderSampler(sampling_strategy=sampling_dict, random_state=42)
    X_resampled, y_resampled = undersampler.fit_resample(X_train, y_train)
    
    # 下采样后分布
    n_class0_after = (y_resampled == 0).sum()
    n_class1_after = (y_resampled == 1).sum()
    print(f"下采样后 - 标签0: {n_class0_after}, 标签1: {n_class1_after}")
    print(f"数据量减少: {len(y_train) - len(y_resampled)} 个样本")
    print(f"最终数据量: {len(y_resampled)} 个样本")
...
if use_undersampling:
    train_data, train_labels = apply_undersampling(train_data, train_labels)
else:
    print("未使用下采样")
...
heatmap(label_true=targets,
        label_pred=pred,
        label_name=[0, 1],
        title="Confusion Table of CNN (Undersampling)",
