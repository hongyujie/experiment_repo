#!/usr/bin/env python3
"""
CNN模型 - 组合采样版本（SMOTEENN方法）
结合上采样和下采样的优点，获得更好的平衡效果
采样前 - 标签0: 11430, 标签1: 2897
采样后 - 标签0: 11351, 标签1: 11427
"""
...
# 超参数
model_name = "CNN_CombinedSampling"
batch_size = 128
epoch = 25
learning_rate = 0.001
dropout = 0
...
def apply_combined_sampling(X_train, y_train):
    """应用SMOTEENN组合采样"""
    print("应用SMOTEENN组合采样...")
    
    # 原始数据分布
    n_class0 = (y_train == 0).sum()
    n_class1 = (y_train == 1).sum()
    print(f"采样前 - 标签0: {n_class0}, 标签1: {n_class1}")
    ...
    print(f"采样后 - 标签0: {n_class0_after}, 标签1: {n_class1_after}")
    print(f"数据量变化: {len(y_resampled) - len(y_train)} 个样本")
...
# 应用组合采样
if use_combined_sampling:
    train_data, train_labels = apply_combined_sampling(train_data, train_labels)
else:
    print("未使用组合采样")
...
heatmap(label_true=targets,
        label_pred=pred,
        label_name=[0, 1],
        title="Confusion Table of CNN (Combined Sampling)",
