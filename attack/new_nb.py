'''数据处理采用混合采样（上采样少数类+下采样多数类）使用朴素贝叶斯分类器进行分类（修复数据泄露）'''
...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/roc_new_nb.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/confusion_matrix_new_nb.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def main():
    ...
    print("\n=== 模型训练完成 ===")
    print("ROC曲线已保存为: fig/roc_new_nb.png")
    print("混淆矩阵已保存为: fig/confusion_matrix_new_nb.png")
