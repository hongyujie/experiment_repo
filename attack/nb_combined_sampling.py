'''数据处理采用混合采样（上采样少数类+下采样多数类）使用朴素贝叶斯分类器进行分类'''
...
# 训练朴素贝叶斯模型
def train_nb_model(X_train, X_test, y_train, y_test):
    """
    训练朴素贝叶斯模型并评估性能
    """
    # 创建朴素贝叶斯分类器
...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    plt.title('朴素贝叶斯模型ROC曲线（混合采样）')
    plt.legend(loc="lower right")
    plt.grid(True, alpha=0.3)
    
    # 保存图片到fig文件夹
    plt.savefig('fig/roc_nb_combined.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.title('Confusion Table of Naive Bayes (Combined Sampling)')
    ...
    # 保存图片到fig文件夹
    plt.savefig('fig/confusion_matrix_nb_combined.png', bbox_inches='tight', dpi=300)
    plt.show()
...
def main():
    """
    主函数：执行完整的朴素贝叶斯模型训练流程（混合采样）
    """
    print("=== 朴素贝叶斯二分类模型训练（混合采样） ===\n")
    ...
    # 5. 训练朴素贝叶斯模型
    print("\n步骤5: 训练朴素贝叶斯模型")
    y_pred, y_pred_proba = train_nb_model(X_train, X_test, y_train, y_test)
    ...
    print("\n=== 模型训练完成 ===")
    print("ROC曲线已保存为: fig/roc_nb_combined.png")
    print("混淆矩阵已保存为: fig/confusion_matrix_nb_combined.png")
