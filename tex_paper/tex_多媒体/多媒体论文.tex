...
{\noindent \bfseries \zihao{-5} 摘\quad 要：}多模态行人重识别旨在突破单模态识别的场景局限，通过融合可见光图像、红外图像、文本语义、视频序列等多源模态信息，实现复杂环境下的精准行人身份匹配，是智能安防与计算机视觉领域的核心技术。本文系统梳理多模态行人重识别的研究体系：首先界定模态、跨模态匹配、模态鸿沟等核心概念，划分涵盖可见光 - 红外、文本 - 图像、视频 - 单帧的双模态跨模态任务，以及多源融合任务核心任务，建立以 Rank-k 准确率、mAP 为核心、MMD 为辅助的评价指标体系；其次按技术路线综述五大核心方法，包括特征对齐与融合、注意力与特征增强、语义与无监督学习、视频多模态融合及模态差异消除，深入分析各类方法的技术逻辑与优势；最后总结当前面临的模态鸿沟缓解、动态融合适配、低资源场景泛化及工程化部署四大挑战，并展望多模态大模型融合、自适应差异消除、低资源学习优化及跨平台适配等未来方向。
...
\indent 模态差异消除是跨模态行人（车辆）重识别的核心挑战，2024-2025 年相关英文研究通过创新特征分离、局部对齐等策略高效缩小模态鸿沟：Seo 等$^{[51]}$2024 年提出基于关键点掩码与擦除的特征表示方法，通过关键点掩码归一化局部特征表示以减少特征级模态差异，搭配关键点擦除提升全局特征多样性与有效性，在 SYSU-MM01 数据集上验证了对相似外观行人匹配的优化效果；Shi 等$^{[52]}$2024 年提出跨模态信息瓶颈表示学习网络（CM InfoNet），通过互信息瓶颈权衡筛选身份相关信息、压缩冗余内容，设计模态共识模块对齐可见光与红外特征，同时获取全局 - 局部特征以强化关键部位判别，有效消除跨模态与模态内变异；MCGS-ReID 团队$^{[53]}$2024 年提出模态交叉图采样（MCGS）方法，助力网络挖掘更多跨模态信息，搭配多模态共享特征对齐网络强化跨模态共享特征表示，实现不同模态特征精准对齐，在 VT-Vehicle 等数据集上显著缓解成像原理与光谱差异导致的模态鸿沟；

% 插入技术对比表格
\begin{center}
    \captionof{table}{跨模态行人重识别技术路线对比}
    
    \resizebox{\linewidth}{!}{% 调整表格大小
    \begin{tabular}{ccccc}
    \toprule
    \textbf{技术类别} & \textbf{核心思路} & \textbf{关键创新点} & \textbf{优势} & \textbf{局限性} \\
    \midrule
    特征对齐与融合 & 分离共享/特异性特征，通过多粒度/频域融合强化一致性 & 频域-空域协同、分层聚合 & 缓解姿态/遮挡干扰 & 计算开销较大 \\
    注意力与特征增强 & 聚焦关键区域，抑制冗余，强化模态间通道交互 & 跨通道注意力、内容感知机制 & 提升特征判别性，轻量化设计可落地 & 对极端遮挡鲁棒性不足 \\
    语义与无监督学习 & 语义对齐+伪标签生成，降低标注依赖 & CLIP改进、MLLMs语义细化 & 适配低资源场景 & 伪标签噪声影响性能 \\
    视频跨模态融合 & 挖掘时空信息，协同静态外观与动态运动线索 & 多粒度时空交互、运动不变记忆 & 动态场景鲁棒性强 & 需视频序列数据，采集成本高 \\
    模态差异消除 & 特征解纠缠/局部对齐，缩小模态分布偏移 & 关键点掩码、信息瓶颈筛选 & 直接缓解模态鸿沟 & 对动态分布适配不足 \\
    \bottomrule
    \end{tabular}}
\end{center}
...
\indent 多模态行人重识别作为智能安防与计算机视觉领域的核心技术，旨在突破单模态识别的场景局限，通过融合可见光、红外、文本、视频等多源信息，实现复杂环境下的精准行人身份匹配，为全天候智能监控、无图寻人、动态追踪等任务提供核心支撑。本文系统梳理了该领域的发展历程、基础框架与核心技术，形成了全面的综述体系。
在基础框架层面，本文明确了多模态 ReID 的核心概念与模态特性，将核心任务划分为可见光 - 红外、文本 - 图像、视频跨模态、多源融合四类，建立了以 Rank-k 准确率、mAP 为核心，MMD 为辅助的评价指标体系，为技术对比与性能衡量提供标准。在核心技术层面，本文按技术路线将现有方法划分为五大类：特征对齐与融合类通过多尺度、频域聚合等策略强化特征一致性；注意力与特征增强类借助多样化注意力机制聚焦关键信息；语义与无监督类通过语义挖掘与低监督学习降低标注依赖；视频多模态融合类利用时空信息提升动态场景鲁棒性；模态差异消除类通过特征分离、局部对齐等策略缩小模态鸿沟。各类方法从不同维度缓解了模态异质性带来的挑战，在 SYSU-MM01、RegDB、CUHK-PEDES 等主流数据集上验证了有效性。
...
% % 插入单张照片示例 - 调整为较小尺寸并使用[h]选项尝试放在当前位置
% \begin{center}
%   \includegraphics[width=0.6\linewidth]{h_CNN.png}% 减小图片宽度
%   \captionof{figure}{这是一个照片示例}
%   \label{fig:photo999}
% \end{center}
...
\noindent[2] Zheng L, Yang Y, Hauptmann A G. Person re-identification: past,present and future[EB/OL].[2020-09-07].\par
...
\noindent[10] Yu H X, Zheng W S, Wu A, et al. Unsupervised person re-identification by soft multilabel
learning[C]//2019 IEEE/CVF Conference on Computer Vision and Pattern Recognition. Piscataway:IEEE, 2019: 2143-2152.\par
...
\noindent[13] Li W, Zhang Y, Wang C. Dual-path convolutional image-text embeddings with instance loss for
text-based person retrieval[C]//2017 ACM Multimedia Conference. New York: ACM, 2017: 1544-1551.\par
\noindent[14] 周勇，王瀚正，赵佳琦，等．基于可解释注意力部件模型的行人重识别方法 [J]. 自动化学
报,2023,49(10): 2159-2171.\par
\noindent[15] Ye M, Shen J, Lin G, et al. Deep learning for person re-identification: a survey and outlook[J].
IEEE Transactions on Pattern Analysis and Machine Intelligence, 2022, 44(6): 2872-2893.\par
\noindent[16] Nguyen D T, Hong H G, Kim K W, et al. Person recognition system based on a combination of
body images from visible light and thermal cameras[J]. Sensors, 2017, 17(3): 605.\par
...
\noindent[19] Chen S, Liu J, Zhang H, et al. CASIA Gait Dataset B: A multi-modal gait database for person
re-identification[J]. Pattern Recognition, 2019, 90: 187-198.\par
\noindent[20] Lin Y, Zheng L, Zheng Z, et al. A comprehensive evaluation of person re-identification metrics[J].
Pattern Recognition, 2023, 139: 109452.\par
\noindent[21] Liu J, Sun Y, Zhu F, et al. Learning memory-augmented unidirectional metrics for cross-modality
person re-identification[C]//2022 IEEE/CVF Conference on Computer Vision and Pattern Recognition. Piscataway: IEEE, 2022: 19366-19375.\par
\noindent[22] Li X, Wang Y, Zhang Z. Cross-Modal Feature Alignment via Dual Attention Guided Fusion for
Person Re-identification [C]//Proceedings of the IEEE/CVF Conference on Computer Vision and
Pattern Recognition. 2024: 12456-12465.\par
\noindent[23] 程德强，钱建生，郭星歌，等。基于多粒度融合和跨尺度感知的跨模态行人重识别 [J]. 通信学报,2025, 46 (1): 110-123.\par
\noindent[24] Zhang H, Li J, Chen W. Hierarchical Feature Fusion with Modal Consistency Constraints for
Cross-Modal Person ReID [C]//Proceedings of the European Conference on Computer Vision. 2024:8912-8921.\par
...
\noindent[29] 朱沛伍，高树辉。低高频多尺度融合的跨模态行人重识别研究 [J]. 重庆邮电大学学报 (自然科学
版), 2024, 36 (6): 1183-1192.\par
...
\noindent[34] Kim S, Park J, Lee D. Attention-enhanced multimodal feature fusion network for clothes-changing
person re-identification [J]. Complex \& Intelligent Systems, 2024, 10 (5): 4871-4885.\par
...
\noindent[38] Liu H, He L, Zhang Y. Semantic-Aligned Learning with Collaborative Refinement for Unsupervised
VI-ReID [J]. International Journal of Computer Vision (IJCV), 2025, 133 (4): 987-1005.\par
\noindent[39] 董令赟。基于跨模态语义一致性和半监督的跨模态行人重识别算法研究 [J]. 计算机工程与应用,2024, 60 (18): 145-153.\par
\noindent[40] Pang Y, Chen J, Li M. Augmented and Softened Matching for Unsupervised Visible-Infrared
Person Re-Identification [C]//Proceedings of the IEEE International Conference on Computer Vision(ICCV). 2025: 7632-7641.\par
\noindent[41] 宋存利。基于语义增强网络的跨模态行人重识别 [J]. 模式识别与人工智能,2025, 38 (2): 156-164.\par
\noindent[42] Qin Y, Yang S, Wang Z. Human-centered Interactive Learning via MLLMs for Text-to-Image
Person Re-identification [J]. arXiv preprint arXiv:2506.11036, 2025.\par
\noindent[43] 贾军营。改进 CLIP-ReID 的跨模态行人重识别 [J]. 计算机应用研究,2024, 41 (10): 3078-3083.\par
\noindent[44] Wang C, Zhang L, Liu J. Multi-Memory Matching for Unsupervised Visible-Infrared Person ReIdentification [J]. arXiv preprint arXiv:2401.06825, 2024.\par
\noindent[45] Zhang S, Li K, Zhao H. Exploring Homogeneous and Heterogeneous Consistent Label Associations
for Unsupervised Visible-Infrared Person ReID [J]. arXiv preprint arXiv:2402.00672, 2025.\par
\noindent[46] Yao Y, Li J, Wang Z. Unsupervised Visible-Infrared Person Re-identification under Unpaired
Settings[C]//Proceedings of the IEEE International Conference on Computer Vision (ICCV). 2025:
4567-4576.\par
\noindent[47] Asurada Y, Chen S, Liu M. X-ReID: Multi-granularity Information Interaction for Video-Based
Visible-Infrared Person Re-identification[J]. arXiv preprint arXiv:2511.17964, 2025.\par
\noindent[48] Visuang T, Zhang L, Chen Y. Video-Level Language-Driven Video-Based Visible-Infrared Person
Re-identification[J]. arXiv preprint arXiv:2506.02439, 2025.\par
\noindent[49] Zhang H, Liu Q, Li D. Event-based Video Person Re-identification via Cross-Modality and Temporal Collaboration[J]. arXiv preprint arXiv:2501.07296, 2025.\par
\noindent[50] VCM Project Team. Learning Modal-Invariant and Temporal-Memory for Video-based VisibleInfrared Person Re-Identification[J]. arXiv preprint arXiv:2208.02450, 2025.\par
\noindent[51] Seo J, Kim D, Lee S. Keypoint Mask-based Local Feature Matching and Keypoint Erasingbased Global Feature Representations for Visible-Infrared Person Re-Identification[C]//Proceedings
of Machine Learning Research. 2024, 263: 3546-3561.\par
\noindent[52] Shi H, Luo M, Zhang X Y, et al. Learning Cross-modality Information Bottleneck Representation
for Heterogeneous Person Re-Identification[J]. arXiv preprint arXiv:2308.15063, 2024.\par
\noindent[53] Zhang Y, Li W, Wang C. MCGS-ReID: A Visible-Infrared Vehicle Reidentification Method Using
Modal-Cross Graph Sampler[J]. IEEE Access, 2024, 12: 10801215.\par
